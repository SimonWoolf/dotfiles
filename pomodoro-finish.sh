...
case $? in
  0)
    RESULT=`curl -X POST "https://www.beeminder.com/api/v1/users/coiley/goals/work/datapoints.json?auth_token=$BEEMINDER_TOKEN&value=1&comment=xfce4-timer-$(date +%H:%M)"`
    if [[ $? -ne 0 ]]; then
